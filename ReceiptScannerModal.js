...
// Significantly improved parseReceiptText function with OCR error correction
function parseReceiptText(text) {
  ...
  text = text.replace(/\bS\b/g, '$'); // S -> $
  text = text.replace(/\bB\b/g, '$'); // B -> $ (sometimes)
  text = text.replace(/\bM\b/g, 'x'); // M -> x (quantity multiplier)
  text = text.replace(/\bF\b/g, ''); // Remove stray F characters
  text = text.replace(/\bD\b/g, ''); // Remove stray D characters
  ...
  const ignorePatterns = [
    ...
    /^pan/i, /^aid/i, /^contactless/i, /^payment/i, /^card/i
  ];

  // More flexible price regex that handles various formats
  const priceRegex = /(?:^|\s)[£€$]\s*(\d+(?:\.\d{2})?)|(\d+\.\d{2})\s*[£€$]?(?:\s|$)/;

  // Enhanced quantity patterns with OCR error correction
  const quantityPatterns = [
    /^(\d+)\s*x\s*(.+)/i,        // "2 x Item Name"
    /^(\d+)\s*M\s*(.+)/i,        // "2 M Item Name" (OCR error)
    /^(\d+)\s+(.+)/,             // "2 Item Name"
    /(.+)\s*x\s*(\d+)/i,         // "Item Name x 2"
    /(.+)\s+M\s*(\d+)/i,         // "Item Name M 2" (OCR error)
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      ...
      <View style={[styles.overlay, { backgroundColor: theme.overlay }]}>
        <View style={[styles.modal, { backgroundColor: theme.surface }]}>
          ...
          {image && !loading && !editModalVisible && ocrText && (
            <View style={{ maxHeight: 400 }}>
              <View style={{ flexDirection: 'row', marginBottom: 16 }}>
                ...
                <View style={{ flex: 1, paddingLeft: 12 }}>
                  <Text style={[styles.ocrText, { color: theme.textSecondary, fontSize: 16, fontWeight: '600' }]}>
                    OCR Text:
                  </Text>
                  <ScrollView style={{ maxHeight: 280, backgroundColor: theme.background, borderRadius: 8, padding: 10, marginTop: 8 }}>
